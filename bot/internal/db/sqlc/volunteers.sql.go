...
RETURNING id, cv, search_radius, category_ids
`
...
SELECT id, cv, search_radius, category_ids
FROM volunteers
WHERE id = $1
`
...
SELECT v.id, v.cv, v.search_radius, v.category_ids, u.username, u.name, u.role, u.state, u.location_lat, u.location_lon
FROM volunteers v
...
func (q *Queries) UpdateVolunteerProfile(ctx context.Context, arg UpdateVolunteerProfileParams) (Volunteer, error) {
	row := q.db.QueryRow(ctx, updateVolunteerProfile, arg.Cv, arg.SearchRadius, arg.ID)
	var i Volunteer
	err := row.Scan(
		&i.ID,
		&i.Cv,
